 ...
 */
public class TestWeakConnectivityAction extends AutomatonAction {
    /**
     ...
     */
    public TestWeakConnectivityAction(String s, Automaton a, Environment e) {
        super(s, null);
        automaton = a;
        environment = e;
    }
    ...
    /**
     * Check if a path exists between every pair of states ignoring edge directions
     */
    public void actionPerformed(ActionEvent e) {
        ...
        while (queue.size() > 0) {
            ...
            for (Transition t : automaton.getTransitionsToState(curr))
                neighborhood.add(t.getFromState());
        ...
        if (visited.size() == states.length) {
            JOptionPane.showMessageDialog(environment,
                    "The graph is weakly connected", "Testing Connectivity",
                    JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(environment,
                    "The graph is not weakly connected", "Testing Connectivity",
                    JOptionPane.INFORMATION_MESSAGE);
        }
    }
}
